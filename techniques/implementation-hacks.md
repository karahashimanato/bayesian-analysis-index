...
- **なぜ効くか**: BOの反復ループは「GPを再学習→獲得関数を計算→次の点を選ぶ」を何十回も繰り返す構造であり、フルベイズ推論のコストをそのまま反復回数倍すると非現実的になる。初回だけNUTSでハイパーパラメータの事後分布が健全であることを確認しておけば、以降は点推定で十分実用的な近似になるという、実務的なBO実装で一般的な妥協点。
