...
- **なぜ効くか**: 手法によって、性能を支えている前提(例: Laplace近似のlast-layer版は特徴抽出器をMAP解に固定するという前提)が異なる。低次元・滑らかなタスクではその前提が問題にならなくても、高次元の実データで前提が崩れると急激に性能が悪化しうる(test NLLが-0.35→+2.70まで悪化した事例がある)。トイデータでの比較は出発点として有用だが、それだけで実務適用を判断するのは危険。
- **登場プロジェクト**: [bayesian-deep-learning](https://github.com/karahashimanato/bayesian-deep-learning/blob/main/README.md#part-c-実データへの拡張--california-housing)
...
- **登場プロジェクト**: [bayesian-deep-learning](https://github.com/karahashimanato/bayesian-deep-learning/blob/main/README.md#part-b-4-より根本的な対策--function-space-repulsive-ensembles)
