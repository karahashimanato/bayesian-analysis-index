...
![分散/平均比=62の過分散データにPoissonは全く合わない: 実データは裾が長く伸びるが、Poisson(平均=7.1)が仮定する分布(分散/平均比=1)は狭い範囲に密集する](../assets/eda/overdispersion_check.png)

*負の二項分布(Gamma-Poisson)で分散/平均比≈68を再現した合成データと、同じ平均のPoisson分布を比較した結果(生成スクリプト: [scripts/generate_eda_plots.py](../scripts/generate_eda_plots.py))。*
...
![候補説明変数間の相関ヒートマップ: office-electronics間が0.95と高く、他の組み合わせ(0.26/0.28)より明確に高いため多重共線性のリスクからofficeを候補から除外する](../assets/eda/covariate_correlation_check.png)

*3つの候補説明変数(office/electronics/google_brand相当)の相関を実際に計算した結果(生成スクリプト: [scripts/generate_eda_plots.py](../scripts/generate_eda_plots.py))。*
...
### 欠測パターンの可視化から、欠測メカニズムの仮説とモデル設計の根拠を得る

![欠測パターンの可視化: 人口30万人未満の国では指標1の欠測率が43.5%だが30万人以上では1.9%と大きく異なり(左)、指標1と指標2の欠測フラグは相関0.79と強く共起する(右)](../assets/eda/missingness_pattern.png)

*人口規模と関連する欠測メカニズム(MAR的)、および2指標間の欠測フラグの共起を合成データで再現した結果(生成スクリプト: [scripts/generate_eda_plots.py](../scripts/generate_eda_plots.py))。欠測メカニズムの分類そのものの定義は[tools/missing-data.md](../tools/missing-data.md#mcarmarmnar欠測メカニズムの分類)を参照。*

- **症状**: 欠測を含むデータで、欠測率の集計だけを見て「何%欠けているか」を確認して終わりにすると、欠測がどんな仕組みで起きているか(MCAR/MAR/MNARのどれに近いか)の手がかりを見落とし、後続のモデル選択が勘頼みになる。
- **対処**: 指標ごとの欠測率に加えて、(1)欠測が時間軸で構造的に固定されているか(同一の対象で毎回欠測するか、たまたま特定の年だけ欠けるか)、(2)欠測の有無が他の完全観測変数(人口規模・GDPなど)の分布とどう関係するか、(3)複数の変数の欠測フラグ同士がどれだけ共起するか、の3点をEDA段階で可視化する。bayesian-missing-dataでは、World Bank指標の欠測が国・地域単位で23年間ほぼ固定されていること、GDPよりも人口規模と強く結びつくこと(人口30万人未満で欠測率43.9% vs 以上で1.7%)、2つの健康指標の欠測フラグ同士が強く共起する(相関0.88)ことをEDAで発見し、それぞれ後続notebookの設計根拠(半合成注入の駆動変数、多変量同時モデルの動機)にした。
- **なぜ効くか**: 欠測メカニズムの分類(MCAR/MAR/MNAR)はモデル選択の根幹(完全ケース分析で十分か、多重代入が必要か、欠測メカニズム自体をモデル化する必要があるか)を左右するが、集計された欠測率だけからは判断できない。「誰が」「何と関連して」欠けているかをEDA段階で可視化しておくことで、以降のモデル設計における仮説を具体的な数値に基づいて立てられる。欠測メカニズムの分類そのものの定義は[tools/missing-data.md](../tools/missing-data.md#mcarmarmnar欠測メカニズムの分類)を参照。
- **登場プロジェクト**: [bayesian-missing-data](https://github.com/karahashimanato/bayesian-missing-data/blob/main/README.md#00-eda--欠測パターンの可視化)

---

### 季節性・周期性の有無をEDAで確認し、モデルの成分構成に反映する

![季節性・周期性のEDA: 曜日別の平均件数は平日366件に対し週末240件と明確に異なり(左)、周期図(パワースペクトル)には周期7日に明確なピークが現れる(右)](../assets/eda/seasonality_periodogram.png)

*週次季節性を持つ合成日次カウントデータで、曜日別集計とFFTによる周期図を実際に計算した結果(生成スクリプト: [scripts/generate_eda_plots.py](../scripts/generate_eda_plots.py))。*
