...
- **なぜ効くか**: mean-field近似は変数間の共分散を0とみなすため、相関の強いパラメータ空間では不確実性の過小評価が体系的に起こる。
...
- **なぜ効くか**: トンプソン抽出は理論上は事後分布のサンプリングで自然に探索と活用のバランスを取るが、収縮バイアスなど他の要因で特定の腕への確信が過度に強まると、事後サンプリングだけでは抜け出せない局所解に陥ることがある。強制探索の下限を別途設けることで、この種の失敗モードに対する保険になる。
