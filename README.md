...
| [PyMC/ArviZコーディングパターン](tools/pymc-code-patterns.md) | 12プロジェクト横断で頻出するPyMC/ArviZの実装イディオム(非中心化パラメータ化・prior predictive check・診断チェックなど)に加え、Euler法のODE積分・forward algorithm・ICAR/BYM2のスケーリング係数など低頻度だが見返す価値のある実装も収録したカンニングペーパー |
