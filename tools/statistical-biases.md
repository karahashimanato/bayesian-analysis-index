...
![Ecological Bias: 群レベルの交絡変数により、真の群内効果(beta_within=-0.60)を個体レベル回帰(群固定効果あり)は-0.55と正しく復元するが、群平均どうしを単純回帰した集計レベル推定は+1.21と符号が反転する](../assets/statistical-biases/ecological_bias.png)

*PyMCで実際に群レベルの交絡変数が個体レベルのx・yの両方に影響する疑似データを生成し、群固定効果を入れた個体レベル回帰と、群平均どうしを単純回帰した集計レベル回帰の係数を比較した結果(生成スクリプト: [scripts/generate_statistical_biases_plots.py](../scripts/generate_statistical_biases_plots.py))。*
