...
推定したいパラメータの性質から、どの事前分布族に当たるかを見分けるための特徴質問チャート。各entryの「選び方のコツ」で説明している判断基準を経路にしたもの。

```mermaid
flowchart LR
    Start(["推定したいパラメータは<br/>どんな性質を持つか?"])

    Start -->|"確率・割合(0〜1に制約)"| Prob["確率・割合"]
    Start -->|"正の値で、分散・強度を<br/>分母的に決める役割<br/>(集中度パラメータなど)"| Scale["正のスケール・<br/>集中度パラメータ"]
    Start -->|"変化点の位置(時点)"| CP["変化点位置"]
    Start -->|"無制約の実数<br/>(回帰係数など)"| Real["無制約の実数<br/>パラメータ"]
    Start -->|"正の値で、桁が数倍〜<br/>数十倍変わりうる"| Wide["桁が大きく変わりうる<br/>正の値"]

    Prob -->|"α/βより解釈しやすい<br/>再パラメータ化がしたい"| Beta["Beta(μκ,(1-μ)κ)"]

    Scale -->|"0付近の分散爆発を<br/>構造的に防ぎたい"| Gamma["Gamma(alpha>1,...)"]
    Scale -->|"特に制約は不要"| HN["HalfNormal / HalfCauchy"]

    CP -->|"Compound StepでESSが<br/>下がるのを避けたい"| Sigmoid["Uniform+シグモイド<br/>(連続緩和)"]
    CP -->|"離散のままでよい"| DU["DiscreteUniform"]
```

- [確率・割合](#確率割合01に制約された値) / [正のスケール・集中度パラメータ](#正のスケール集中度パラメータσ-λ-κなど0より大きい値) / [変化点位置](#変化点位置離散-vs-連続の時点パラメータ) / [無制約の実数パラメータ](#無制約の実数パラメータ回帰係数など) / [桁が大きく変わりうる正の値](#桁が大きく変わりうる正の値初期値カウントの推定値など)
