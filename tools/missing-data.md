...
![MICE(m=20)の正規近似95%区間の実測カバレッジ=93.3%(名目の95%を下回る): 60回のシミュレーション反復のうち、真値を含まない区間(オレンジ)が複数発生する](../assets/missing-data/mice_coverage.png)

*単一変数のMICE(回帰による予測+残差ノイズでの補完、Rubinのルールで統合)を60回繰り返しシミュレーションした結果(生成スクリプト: [scripts/generate_missing_data_plots.py](../scripts/generate_missing_data_plots.py))。*
...
![Selection Model: 観測率23%のMNARデータから、除外制約変数を使ってgamma_y(観測確率がyの値自体に依存する強さ)を実際に復元できる(事後平均=-0.98、真値=-1.0、divergence=0)](../assets/missing-data/selection_model_recovery.png)

*アウトカム式と選択式(ロジスティック)をPyMCで同時に実際にサンプリングした結果(生成スクリプト: [scripts/generate_missing_data_plots.py](../scripts/generate_missing_data_plots.py))。選択式に除外制約(アウトカム式には含まれない変数)がある場合は良好に識別されるが、除外制約なし(選択式・アウトカム式が同じ共変量を共有)で追試すると、r_hatが1.12まで悪化し不安定になることを確認した(識別の弱さは条件依存)。*
...
![Pattern-Mixture Modelの感度分析: delta*=1.286(真の周辺平均3.702を再現)は真のdelta=1.2に近い値になるが、これはあくまで合成データで真値が既知だから確認できるものであり、実データでは事後的にも分からない](../assets/missing-data/pattern_mixture_sensitivity.png)

*欠測群の分布に感度パラメータdeltaを加えた合成データで、deltaを動かしたときの周辺平均推定の変化を実際に計算した結果(生成スクリプト: [scripts/generate_missing_data_plots.py](../scripts/generate_missing_data_plots.py))。*
...
![多変量同時欠測モデル: 相関rho=0.7の2変数で、y1との相関を利用する同時モデルは独立モデルより補完RMSEを23.1%改善する(1.474→1.134)](../assets/missing-data/multivariate_joint_imputation.png)

*相関する2変数(y1完全観測、y2に30%欠測)を、独立モデルと逐次条件付け分解による同時モデルの両方でPyMCで実際にフィットし、真の欠測値とのRMSEを比較した結果(生成スクリプト: [scripts/generate_missing_data_plots.py](../scripts/generate_missing_data_plots.py))。*
